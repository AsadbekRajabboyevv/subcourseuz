...
public interface TestRepository extends BaseRepository<TestEntity, Long> {
    ...
    @Query("""
            SELECT new uz.asadbek.subcourse.test.test.dto.TestResponseDto(
                t.id,
                t.name,
                t.description,
                t.price,
            ...
            AND (
                :#{#filter.scienceName} IS NULL OR\s
                LOWER(s.name.nameCrl) LIKE LOWER(CONCAT('%', :#{#filter.scienceName}, '%')) OR
                LOWER(s.name.nameEn) LIKE LOWER(CONCAT('%', :#{#filter.scienceName}, '%')) OR
                LOWER(s.name.nameRu) LIKE LOWER(CONCAT('%', :#{#filter.scienceName}, '%')) OR
                LOWER(s.name.nameUz) LIKE LOWER(CONCAT('%', :#{#filter.scienceName}, '%'))
            )
